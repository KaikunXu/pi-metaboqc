...
"""
Script purpose: Provide PCA workflows and multivariate outlier diagnostics.

This module prepares transformed and scaled intensity matrices, runs
scikit-learn PCA, and returns score/loadings structures compatible with
MetaboInt assessment plots. It computes exact Hotelling's T2 and SPE/DModX
limits for statistical outlier labeling, then reports dispersion, silhouette,
centrality-shift, and explained-variance diagnostics.
The PCAEngine keeps dimensionality-reduction logic separate from plotting and
...
class PCAEngine:
    ...
    def extract_features(
        ...
        subset_df = su.apply_feature_scaling(df=subset_df, method=scaling_method)
        ...
        feature_columns = list(set(feature_dataframe.index.names) - {sample_name})
    ...
    def run_pca_workflow(self, features: pd.DataFrame) -> PCAWorkflowResult:
        ...
        model = PCA(n_components=self.n_components, random_state=self.global_seed)
        ...
        cond_orthogonal = (metrics["SD"] <= sd_limit) & (metrics["OD"] > od_limit)
    ...
    def _compute_exact_limits(
        ...
        sd_limit = float(
            (self.n_components * (n_samples - 1) / (n_samples - self.n_components))
            * f_critical_value
        )
        ...
        if self.od_method == "box":
            ...
            if spe_variance > 1e-10:
                ...
                od_limit = float(g_factor * chi2.ppf(1 - self.alpha, df=h_factor))
            else:
    ...
    def calc_relative_dispersion(
        ...
        return (
            float(qc_variance / actual_variance) if actual_variance > 1e-9 else np.nan
        )

    @staticmethod
    def calc_qc_batch_silhouette(
        coords: np.ndarray, types: np.ndarray, batches: np.ndarray, qc_label: str
    ) -> float:
    ...
    def calc_qc_centrality_shift(
        ...
        actual_distances = np.linalg.norm(actual_coordinates - actual_centroid, axis=1)
...
if __name__ == "__main__":
    ...
    # 2. Initialize Engine
    engine = PCAEngine(n_components=2, alpha=0.05, od_method="box", global_seed=123)
