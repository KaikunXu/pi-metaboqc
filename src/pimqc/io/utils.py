"""Shared runtime, configuration, file-system, and logging utilities.

This module loads and validates TOML or JSON pipeline configurations, manages
output directories and archive helpers, configures Loguru, and provides common
progress, hardware-diagnostic, and serialization utilities. Its functions are
used by the CLI, notebook, pipeline, and individual processing stages.
"""
...
def setup_loguru_logger(level: str = "INFO") -> None:
    ...
    def dynamic_formatter(record: dict[str, Any]) -> str:
        ...
        # (e.g., 'pimqc.dataset.builder' -> 'builder')
...
def _load_json_file(input_file_path: str) -> Dict[str, Any]:
    ...
    """
    with open(
        file=input_file_path, mode="r", encoding="utf-8-sig"
    ) as json_file:
...
def _save_json_file(content: Dict[str, Any], output_file_path: str) -> None:
    ...
    """
    with open(
        file=output_file_path, mode="w", encoding="utf-8-sig"
    ) as json_file:
        json.dump(
            obj=content,
            fp=json_file,
...
def load_pipeline_config(config_path: str) -> Dict[str, Any]:
    ...
    if ext == ".json":
        ...
        err_msg = f"Unsupported config format: {ext}. Use JSON or TOML."
