...
def align_paired_matrices(
    ...
    common_samples = before_df.columns.intersection(after_df.columns, sort=False)
    return (
...
def rank_loss_from_distances(raw_dist: np.ndarray, transformed_dist: np.ndarray) -> float:
...
def calc_sample_structure_arrays(
    ...
    if raw_z.shape[0] >= 2:
        ...
        valid_geom = np.isfinite(raw_geom_dist) & np.isfinite(transformed_geom_dist)
        ...
        if n_samples >= 3:
            ...
            for sample_idx in range(n_samples):
                ...
                raw_neighbors = raw_neighbors[raw_neighbors != sample_idx][:n_neighbors]
                ...
                local_trustworthiness.append(float(np.clip(local_score, 0.0, 1.0)))
            empty["geometry"]["sample_neighborhood_trustworthiness"] = pd.Series(
                local_trustworthiness,
                index=raw_z.columns,
                dtype=float,
            )
...
def calc_sample_structure_preservation(
    ...
    median_log2_ratio = finite_or_nan(metrics["median_sample_log2_distance_ratio"])
    ...
    median_relative_delta = finite_or_nan(metrics["robust_distance_relative_delta"])
