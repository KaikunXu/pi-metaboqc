...
"""
Purpose: Invalid feature/sample filtering and visualization suite.
Provides automated topological routing for missing value classification,
low-quality feature removal, and sample-level quality control.
"""
...
class MetaboIntFilter(core_classes.MetaboInt):
    ...
    def execute_sample_filtering(self, output_dir: str = None) -> pd.DataFrame:
        ...
        sort_levels = [
            lvl for lvl in [batch, inject_order] 
    ...
    def _get_valid_bio_groups(self):
        ...
        if bio_group in self.columns.names:
            ...
            for group in raw_groups:
                ...
                if (group_str in self._INVALID_STRS or 
                    group_str == str(qc_label).lower()):
                    continue
    ...
    def classify_missing_types(self):
        ...
        try:
            ...
            if valid_bio_groups:
                ...
                cond_group_mnar = (
                    (na_rate_valid >= mnar_group_mv_tol).any(axis=1) & 
    ...
    def execute_mv_filtering(self, output_dir: str = None) -> pd.DataFrame:
        ...
        if qc_mask.any():
            self.stats["mv_qc_series"] = self.loc[:, qc_mask].isna().mean(
                axis=1
            )
        ...
        if output_dir:
            ...
            df_final.to_csv(os.path.join(
                output_dir, "Filtered_Data_High-MV_Features.csv"))
            df_tracking.to_csv(
                os.path.join(
                    output_dir,
    ...
    def _generate_s1_tracking_table(
        self, qc_mask, idx_mar, idx_mnar, idx_dropped
    ):
        ...
        for feat in self.index:
            ...
            log2_int = np.log2(val_intensity + 1) if pd.notna(
                val_intensity) else np.nan
    ...
    def mv_filtering_metrics(self) -> Dict[str, Any]:
        ...
        if not track_df.empty:
            ...
            sample_retention_rate = round(
                sample_retained/sample_total * 100, 2) if sample_total else 0.0
        ...
        feature_retention_rate = round(
            (feature_retained_count / feature_raw_count) * 100, 2) if (
                feature_raw_count > 0) else 0.0
    ...
    def execute_quality_filtering(
        ...
        if blank_mask.any() and qc_mask.any():
            ...
            pass_blank = ratio_series[
                ratio_series <= blank_qc_ratio_tol
            ].index
    ...
    def _generate_s2_tracking_table(
        ...
        for feat in start_idx:
            ...
            if feat in idx_dropped_blank:
                (
                    blank_check_status, rsd_check_status, stage2_status,
                    sort_order
                ) = ("Failed", "Skipped", "Drop", 0)
            elif feat in idx_dropped_rsd:
                (
                    blank_check_status, rsd_check_status, stage2_status,
                    sort_order
                ) = ("Passed", "Failed", "Drop", 1)
            else:
                (
                    blank_check_status, rsd_check_status, stage2_status,
                    sort_order
                ) = ("Passed", "Passed", "Keep", 2)
...
class MetaboVisualizerFilter(visualizer_classes.BaseMetaboVisualizer):
    ...
    # =========================================================================
    def _plot_sample_mv_stripplot(self, track_df: pd.DataFrame, tol: float, ax=None):
        ...
        sns.stripplot(
            data=df_plot, x="Sample_Type", y="MV_Rate_Pct", 
            hue="Sample_Type", palette=color_dict, jitter=True, 
            size=7, alpha=0.8, edgecolor="k", linewidth=0.5, 
            ax=current_ax, legend=False)
        
        # Threshold line
        tol_pct = tol * 100
        current_ax.axhline(
            tol_pct, color="k", linestyle="--", linewidth=1.5,
        ...
        self._apply_standard_format(
            ax=current_ax, title="Sample Missing Value Ratios",
            xlabel="Sample Type", ylabel="Missing Rate (%)", append_stage=False)
    ...
    def _plot_group_rescue_scatter(
        self, df, max_col, min_col, mnar_group_mv_tol, active_base_tol, ax, title
    ):
        ...
        sns.scatterplot(
            data=df_plot, x=max_col, y=min_col, 
            hue="Step_Status", 
            palette={
                "MNAR (Group)": color_mnar, "Pending": color_pending
            },
            style="Step_Status", markers={"MNAR (Group)": "X", "Pending": "o"},
            alpha=0.8, ax=ax, legend=False, edgecolor="k"
        )
        ...
        ax.plot(
            [0, 100], [0, 100], color='gray', linestyle='-.', 
            alpha=0.5, zorder=1
        )
        ...
        ax.fill_between(
            [tol_max_pct, 105], -5, tol_min_pct, 
            color=color_mnar, alpha=0.2, zorder=0)
        ...
        handles = [
            ...
            mlines.Line2D(
                [], [], color=color_mnar, marker="X", linestyle="", 
                label="MNAR (Group)", markeredgecolor="k"),
            mlines.Line2D(
                [], [], color=color_pending, marker="o", linestyle="", 
                label="Pending", markeredgecolor="k"),
        ...
        self._format_multi_legends(
            ax=ax, group_titles=["Status", "Thresholds"], 
            loc="upper left", start_bbox=(0.05, 1.0), )
        self._apply_standard_format(
            ax=ax, title=title, xlabel="Max Group MV (%)", 
            ylabel="Min Group MV (%)", append_stage=False)
        ax.title.set_weight("bold")
    
    def _plot_qc_rescue_scatter(
        self, df, mnar_qc_mv_tol, mnar_int_threshold, ax, title
    ):
        """
        ...
        def _determine_status(row):
            if "QC" in row["Stage1_Status"]:
                return "MNAR (QC)"
            elif (row["QC_MV_Pct"] > mnar_qc_mv_tol * 100) and (
                mnar_int_threshold is not None) and (
                    row["Log2_Intensity"] <= mnar_int_threshold):
        ...
        sns.scatterplot(
            data=df_plot, x="Log2_Intensity", y="QC_MV_Pct", 
            hue="Step_Status", 
            ...
            alpha=0.75, ax=ax, legend=False, edgecolor="k"
        )
        
        ax.axhline(
            mnar_qc_mv_tol * 100, color="k", linestyle=":", label="MV Cutoff"
        )
        
        if mnar_int_threshold is not None:
            ax.axvline(
                mnar_int_threshold, color="k", linestyle="--", 
                label="Int Cutoff"
            )
            ax.fill_between(
                [ax.get_xlim()[0], mnar_int_threshold], 
                mnar_qc_mv_tol * 100, 100, 
                color=color_mnar, alpha=0.15, zorder=0
            )
        ...
        handles = [
            ...
            mlines.Line2D(
                [], [], color=color_mnar, marker="X", linestyle="", 
                label="MNAR (QC)", markeredgecolor="k"),
            mlines.Line2D(
                [], [], color=color_blocked, marker="v", linestyle="", 
                label="Blocked", markeredgecolor="k"),
            mlines.Line2D(
                [], [], color=color_pending, marker="o", linestyle="", 
                label="Pending", markeredgecolor="k"),
            mlines.Line2D([], [], color="none", label="Thresholds"),
            ...
            mlines.Line2D(
                [], [], color="k", linestyle="--", label="Low Int")
        ]
        ...
        if has_group_info:
            handles.extend([
                ...
                mlines.Line2D(
                    [], [], color="none", marker="o", markersize=8, 
                    label="Larger =\nHigher Min Group MV", 
        ...
        self._apply_standard_format(
            ax=ax, title=title, xlabel="Log2(Median QC Intensity)", 
            ylabel="QC Missing Rate (%)", append_stage=False)
        ax.title.set_weight("bold")

    def _plot_cutoff_histogram(
        self, df, x_col, hue_col, tol, palette, hue_order, ax, title, x_label
    ):
        ...
        sns.histplot(
            data=df, x=x_col, hue=hue_col, multiple="layer",
            palette=palette, hue_order=hue_order,
            bins=np.arange(0, 105, 5), edgecolor="k", alpha=1, ax=ax)
        ...
        handles.extend([
            ...
            for cat in hue_order if cat in palette])
        
        handles.append(mlines.Line2D([], [], color="none", label="Thresholds"))
        handles.append(
            mlines.Line2D(
                [], [], color="k", linestyle=":", 
                label=f"Cutoff ({tol*100:.0f}%)"))
        
        ax.legend(handles=handles)
        self._format_multi_legends(
            ax=ax, group_titles=["Status", "Thresholds"], 
            loc="upper left", start_bbox=(0.5, 1.0))
        
        self._apply_standard_format(
            ax=ax, title=title, xlabel=x_label, 
            ylabel="Feature Count", append_stage=False)
        ax.title.set_weight("bold")

    def _plot_pipeline_flowchart_atom(
        self, df, ax, mnar_group_mv_tol, mnar_qc_mv_tol, active_base_tol,
        has_group_info, margin_left=0.0, margin_right=0.0, margin_top=0.0,
        margin_bottom=0.0
    ):
        """
        ...
        def _node(x, y, text, bg):
            text_color = pu.get_contrast_color(bg)
            bbox = dict(boxstyle=box_style, facecolor=bg, ec="k", lw=1.2)
            ax.text(
                x, y, text, ha="center", va="center", fontsize=11, 
                fontweight="bold", color=text_color, bbox=bbox, zorder=3
            )
            return (x, y)

        def _arrow(posA, posB, style="horizontal"):
            x_a, y_a = posA
            x_b, y_b = posB
    ...
    def plot_mv_filtering_summary_grid(
        ...
        mnar_group_mv_tol: float = None, mnar_qc_mv_tol: float = 0.2, 
        ...
        if has_group_info:
            ...
            self._plot_pipeline_flowchart_atom(
                df=df_curr, ax=ax_flow, mnar_group_mv_tol=mnar_group_mv_tol, 
                mnar_qc_mv_tol=mnar_qc_mv_tol, active_base_tol=active_base_tol, 
                has_group_info=True, margin_right=0.0
            )
            
            # Subplot S1: Group Rescue Scatter
            ax_group_rescue = pw.Brick(figsize=(4, 4), label="s1")
            self._plot_group_rescue_scatter(
                df_curr, "Max_Group_MV_Pct", "Min_Group_MV_Pct", 
                mnar_group_mv_tol, active_base_tol, ax_group_rescue, 
            ...
            self._plot_qc_rescue_scatter(
                df_curr, mnar_qc_mv_tol, mnar_int_threshold, ax_qc_rescue, 
            ...
            self._plot_cutoff_histogram(
                df_curr, "Min_Group_MV_Pct", "Stage1_Status", 
                active_base_tol, {"MAR": "tab:red", "INVALID": "tab:gray"}, 
                ["MAR", "INVALID"], ax_base_check, "Min Group-level MV Check", 
                "Min Group-level MV (%)"
            )
            ...
            self._plot_pipeline_flowchart_atom(
                df=df_curr, ax=ax_flow, mnar_group_mv_tol=None, 
                mnar_qc_mv_tol=mnar_qc_mv_tol, active_base_tol=active_base_tol, 
            ...
            ax_base_check = pw.Brick(figsize=(4, 4), label="s3")
            self._plot_cutoff_histogram(
                df_curr, "QC_MV_Pct", "Stage1_Status", 
                active_base_tol, {"MAR": "tab:red", "INVALID": "tab:gray"}, 
                ["MAR", "INVALID"], ax_base_check, "QC-level MV Check", 
                "QC-level MV (%)"
            )
    ...
    # =========================================================================
    def _plot_retained_count_steps(self, ax=None):
        ...
        inv_base = max(
            0, feature_counts.get("raw", 0) - (mar_base + mnar_base)
        )
        ...
        pu.show_values_on_bars(
            axs=current_ax, value_format="{:.0f}", fontsize=8,
            stacked=True, skip_zero=True, threshold_pct=0.05
        )
        
        self._apply_standard_format(
            ax=current_ax, title="Feature Retention Across Filtering Steps",
            xlabel="Filtering Steps", ylabel="Feature Count",append_stage=False
        )
    ...
    def _plot_qc_blank_scatter(self, ax=None):
        ...
        idx_mar = pd.Index(self.engine.stats.get("idx_mar", []))
        ...
        sns.scatterplot(
            data=df_plot, x="QC", y="Blank", ax=current_ax, hue="Status",
        ...
        current_ax.plot(
            x_line, np.log2(((2**x_line - 1) * blank_qc_ratio_tol) + 1), 
            color="k", linestyle="--", linewidth=1.5, 
            label=f"Ratio={blank_qc_ratio_tol}"
        )

        self._apply_standard_format(
            ax=current_ax, title="Blank/QC Check",
    ...
    def _plot_rsd_dist(self, idx_mnar, ax=None):
        ...
        sns.histplot(
            data=df_plot, x="RSD", hue="Feature Type", 
        ...
        current_ax.axvline(
            x=qc_rsd_tol, color="k", linestyle="--", linewidth=1.5
        )
        
        handles = []
        if "MAR" in df_plot["Feature Type"].values:
            handles.append(mpatches.Patch(
                facecolor="tab:gray", edgecolor="k", linewidth=1.0, 
                label="MAR"
            ))
        if "MNAR" in df_plot["Feature Type"].values:
            handles.append(mpatches.Patch(
                facecolor="tab:red", edgecolor="k", linewidth=1.0, 
                label="MNAR"
            ))
        ...
        handles.append(
            mlines.Line2D(
                [], [], color="k", linestyle="--", 
                label=f"MAR Threshold ({qc_rsd_tol})", linewidth=1.0
            )
        )
        ...
        self._apply_standard_format(
            ax=current_ax, title="QC RSD Check", 
            xlabel="RSD", ylabel="Feature Count", append_stage=False
        )
