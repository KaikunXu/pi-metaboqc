...
"""
Script purpose: Validate user configuration for the pi-metaboqc pipeline.

This module defines the Pydantic models that describe dataset construction,
assessment, filtering, correction, imputation, normalization, and report
settings. It normalizes user-provided TOML or JSON configuration into a strict
schema before any expensive processing starts.
The validators keep option names, thresholds, feature labels, and processing
modes consistent across the pipeline, reducing silent configuration errors.
"""
...
class MetaboIntConfig(BaseModel):
    ...
    sample_dict: Dict[str, str] = Field(
        default={"Actual sample": "Sample", "Blank sample": "Blank", "QC sample": "QC"}
    )
...
class AssessorConfig(BaseModel):
    ...
    scaling_method: Literal["Auto-scaling", "Pareto-scaling", "None"] = "Auto-scaling"
...
class PipelineConfig(BaseModel):
    ...
    MetaboIntNormalizer: NormalizerConfig = Field(default_factory=NormalizerConfig)
