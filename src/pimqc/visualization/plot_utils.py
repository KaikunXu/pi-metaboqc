...
This module supplies color, typography, axis, legend, dashboard, and
sample-structure utilities shared across visualizers. Its helpers keep compact
figures, vector exports, and report components visually consistent without
owning stage-specific calculations or workflow decisions.
"""
...
def heatmap_annotation_fontsize(
    ...
    n_rows: int,
    n_cols: int,
    default_size: float = DEFAULT_ANNOTATION_FONTSIZE,
    max_size: float = DEFAULT_ANNOTATION_FONTSIZE,
    min_size: float = 4.0,
...
def place_annotation_in_least_occupied_corner(
    ...
    blocked_regions: list[tuple[float, float, float, float]] | None = None,
) -> None:
    """Move an annotation to the least occupied corner of an axis.

    ``blocked_regions`` are axes-coordinate rectangles (x0, y0, x1, y1),
    used for fixed artists such as inset colorbars that should not be covered.
    """
    valid_arrays = [
    ...
    try:
        ...
        text_width, text_height = 0.46, 0.18

    data_points = np.vstack(valid_arrays)
    ...
    for rank, candidate in enumerate(candidates):
        ...
        blocked_area = 0.0
        for bx0, by0, bx1, by1 in blocked_regions or []:
            blocked_area += max(0.0, min(x1, bx1) - max(x0, bx0)) * max(
                0.0, min(y1, by1) - max(y0, by0)
            )
        score = (
            float(np.count_nonzero(in_box))
...
def plot_sample_structure_change_map(
    ...
    sample_shift = geometry.get(
        "sample_log2_distance_ratio", pd.Series(dtype=float)
    )
    sample_rank = geometry.get(
    ...
    annotation_size = DEFAULT_ANNOTATION_FONTSIZE
    ...
    score_note_artist = None
    if score_note:
        score_note_artist = ax.text(
            0.98,
            0.03,
            "\n".join(score_note),
            transform=ax.transAxes,
    ...
    if score_note_artist is not None:
        place_annotation_in_least_occupied_corner(
            ax=ax,
            text_artist=score_note_artist,
            occupancy_arrays=[plot_df[["scale_shift", "rank_rho"]].to_numpy()],
            blocked_regions=[(0.87, 0.08, 1.02, 0.52)],
        )
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    change_fontsize(ax)
...
def plot_sample_distance_scale_shift_panel(
    ...
    The panel is intentionally label-free: it uses actual samples only by
        default
    ...
    median_rel_delta = su.finite_or_nan(
    ...
    if compact_annotations:
        ...
        if np.isfinite(median_rel_delta):
            metric_note.append(
                f"Median relative |delta d|: {median_rel_delta:.3f}"
            )
    ...
    compact_note_fontsize = DEFAULT_ANNOTATION_FONTSIZE
    ...
    ax.axvline(
        0, color="0.20", linestyle="--", linewidth=ref_linewidth, zorder=2
    )
    ...
    ax.annotate(
        ...
        arrowprops={
            ...
            "mutation_scale": 5
            if compact_style
            else (6 if compact_annotations else 9),
            "shrinkA": 0,
            "shrinkB": 0,
        },
