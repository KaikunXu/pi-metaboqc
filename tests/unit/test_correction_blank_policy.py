...
def test_ruviii_fits_only_nonblank_rows_and_projects_blanks() -> None:
    """Keep nonblank RUV fits invariant to changes in blank intensity."""
...
def test_waveica2_excludes_blank_signal_and_predicts_frozen_artifact() -> None:
    """Exclude blank signal from WaveICA component estimation."""
    rng = np.random.default_rng(DEFAULT_RANDOM_SEED)
    columns = [f"S{i}" for i in range(12)]
    data = pd.DataFrame(
        rng.lognormal(mean=5.0, sigma=0.2, size=(5, len(columns))),
        columns=columns,
    )
    blank_mask = np.array(
        [
    ...
    original = WaveICA2Corrector(
        n_components=3,
        cutoff=0.0,
        random_state=DEFAULT_RANDOM_SEED,
    )
    original_df = original.fit_transform(data, **kwargs)["WaveICA 2.0"][0]
    changed = WaveICA2Corrector(
        n_components=3,
        cutoff=0.0,
        random_state=DEFAULT_RANDOM_SEED,
    )
    changed_df = changed.fit_transform(changed_blanks, **kwargs)["WaveICA 2.0"][
        0
    ]
