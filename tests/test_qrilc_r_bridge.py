...
def _make_left_censored_log_matrix() -> tuple[pd.DataFrame, np.ndarray]:
    """Create a log-scale matrix where missing values represent low abundance."""
...
def test_qrilc_matches_imputelcmd_distributional_reference() -> None:
    ...
    py_df = MetaboIntImputer.impute_by_qrilc(
        masked_df, tune_sigma=1.0, global_seed=123
    )
    r_df = run_r_qrilc(masked_df, tune_sigma=1.0, seed=123)
    ...
    assert _relative_mae(r_all, py_all) < 0.03
    assert _relative_mae(r_missing, py_missing) < 0.25
    np.testing.assert_allclose(
