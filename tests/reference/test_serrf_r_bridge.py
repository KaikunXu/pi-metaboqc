...
from pimqc.processing.correction import SERRFCorrector

from .helpers import median_qc_rsd, require_r_package


def _make_serrf_matrix() -> tuple[
    ...
    arr = base[:, None] * (
        1.0 + feature_weight[:, None] * (drift[None, :] - 1.0)
    )
    ...
    return (
        pd.DataFrame(arr, index=index, columns=columns),
        batch,
        qc_mask,
        order,
    )
...
def run_r_serrf(
    ...
    require_r_package("randomForest")
    r_script = """
    function(df, batch, qc_mask, order, n_trees, seed) {
...
def test_serrf_matches_r_randomforest_correction_contract() -> None:
    ...
    raw_rsd = median_qc_rsd(df_input, qc_mask)
    ...
    py_rsd = median_qc_rsd(py_df, qc_mask)
    r_rsd = median_qc_rsd(r_df, qc_mask)
    assert py_rsd < raw_rsd
    assert r_rsd < raw_rsd

    corr = spearmanr(
        py_df.to_numpy().ravel(), r_df.to_numpy().ravel()
    ).correlation
