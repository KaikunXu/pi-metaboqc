...
* **Adaptive correction and normalization selection:** For signal correction, the pipeline compares QC-anchored regression methods (QC-RLSC, QC-RFSC, and QC-SVR) with global model correction methods (SERRF, RUV-III, and WaveICA 2.0), using internal OOF evaluation where applicable and global refitting for final output. For normalization, it evaluates common strategies including robust log transformation, TIC, median, PQN, MDFC, quantile normalization, and VSN with QC-, RLE-, MA-, and sample-structure-aware guardrails.
